...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    N,T,H = dh.shape
    D = cache[0][0].shape[1] # cache[n][0] is x
    dx = np.empty((N,T,D))
    dx[:,T-1,:], dh0, dWx, dWh, db = rnn_step_backward(dh[:,T-1,:], cache[T-1])
    for i in range(T-2,-1,-1):
      dh_i = dh[:,i,:] + dh0
      dx[:,i,:], dh0, dWx_i, dWh_i, db_i = rnn_step_backward(dh_i, cache[i])
      dWx += dWx_i
      dWh += dWh_i
      db  += db_i
    ##############################################################################
